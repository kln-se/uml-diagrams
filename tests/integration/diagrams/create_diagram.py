...
def test_create_diagram_by_user(client: APIClient, logged_in_user: User) -> None:
    ...
    response = client.post(path=DIAGRAMS_URL, data=data)
    assert response.status_code == status.HTTP_201_CREATED
    diagram = Diagram.objects.get(id=response.data["id"])
    assert diagram.owner == logged_in_user
    assert diagram.title == response.data["title"] == data["title"]
    assert diagram.description == response.data["description"] == data["description"]
    assert diagram.json == response.data["json"] == loads(data["json"])
    assert (
